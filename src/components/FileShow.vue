  ...
  <div class="file-show-list-wrap">
    <div v-if="fileList && fileList.length > 0"> 
      <el-tag
        ...
        <!-- 图片 -->
        <a :style="!isDownload ? 'pointer-events: none' : ''" :href="`${common.publicBasePath}${tag.path}`" target="_blank" :download="tag.name" v-if="
          tag.name.split('.')[1]==='jpg'||
          tag.name.split('.')[1]==='png'||
          tag.name.split('.')[1]==='jpeg'||
          tag.name.split('.')[1]==='gif'||
          tag.name.split('.')[1]==='bmp'">
          <img :src="`${common.publicBasePath}${tag.path}`" :onerror="errimage" class="thumbnail fl"/>
          <span class="fl name">{{tag.name}}</span>
        </a>
        <!-- 视频-->
        <a :style="!isDownload ? 'pointer-events: none' : ''" :href="`${common.publicBasePath}${tag.path}`" :download="tag.name" v-else-if="
        ...
        <!-- 其它文件 -->
        <a :style="!isDownload ? 'pointer-events: none' : ''" :href="`${common.publicBasePath}${tag.path}`" :download="tag.name"  v-else>
