  ...
  <div class="content-wrap-box">
    ...
    <div class="router-view-wrap transparent-bg" v-loading="loading">
      ...
      <div class="router-view-content">
        <div v-if="common.checkAuthority('Lesson-Data-QuestionDetail-Query', true)">
          <!-- 基本信息 -->
          <el-card>
            <div slot="header">
            ...
            <el-form label-width="100px" class="show-detail-form"  v-if="courseDetailData">
              <el-row :gutter="20">
                ...
                <el-col :span="24">
                  <p class="text-right">
                    <el-button 
                      :disabled="!common.checkAuthority('Lesson-Data-QuestionDetail-Edit', true) "
                      type="primary" @click="editCourseDialog = true;echo();">
                        编辑
                    </el-button>
                    <el-button 
                      :disabled="!common.checkAuthority('Lesson-Data-QuestionDetail-Delete', true)"
          ...
          <el-card class="mart20">
            <div slot="header" class="overflow-hidden">
              <span class="card-title fl">问卷详情</span>
            </div>
            <div v-if="exercises">
              <!-- 基础信息 -->
              <el-card>
                ...
                <el-form label-width="100px" class="show-detail-form">
                  <el-row :gutter="20">
                    <el-col :span="12">
                      <el-form-item label="名称：">{{exercises.name}}</el-form-item>
                    </el-col>
                    <el-col :span="12">
                      <el-form-item label="类型：">
                        <el-tag type="success" v-if="exercises.type===1">评价教学</el-tag>
                        <el-tag type="primary" v-if="exercises.type===2">互评</el-tag>
                        <el-tag type="warning" v-if="exercises.type===3">自评</el-tag>
                      </el-form-item>
                    </el-col>
                    <el-col :span="24">
                      <el-form-item label="填写要求："><p class="cursor-pointer_T">{{exercises.description}}</p></el-form-item>
                    </el-col>
                  </el-row>
                </el-form>
              </el-card> 
              ...
              <el-card class="mart20">
                <div slot="header" class="overflow-hidden">
                  <span class="card-title fl">问题</span>
                  <el-button 
                    class="fr"
                    v-if="questions.rows.length>0"
                    size="small"
                    :disabled="isDisabaledNexBtn()"
                    type="primary" @click="exercisesIndex+=1;getQueryOPAnalysis(exercises.id,questions.rows[exercisesIndex-1].id,questions.rows[exercisesIndex-1].type)">
                      下一题
                  </el-button>
                  <div class="fr marr10 marl10 mart8 text-bold" v-if="questions.rows.length>0">{{exercises.questionCount>0?exercisesIndex:'0'}}/{{exercises.questionCount}}</div>
                  <el-button 
                    class="fr"
                    v-if="questions.rows.length>0"
                    size="small"
                ...
                <div v-if="exercises.questionCount>0">
                  <div v-for="(item,index) in questions.rows" v-if="index+1===exercisesIndex">
                    <el-form label-width="100px" class="show-detail-form">
                      <el-row :gutter="20">
                        <el-col :span="24">
        ...
        <el-form ref="courseForm" :model="courseForm" label-width="100px" v-loading="dialogLoading">
          ...
          <el-form-item 
            ...
            <el-radio-group v-model="courseForm.genre">
              <el-radio :label="1">评价教学</el-radio> 
              <el-radio :label="2">互评</el-radio>
              <el-radio :label="3">自评</el-radio>
            </el-radio-group>
          </el-form-item> 
          <el-form-item 
            label="要求：" 
  ...
  export default {
    data () {
      return { 
        ...
        exercises:         null,// 任务的问卷基本信息
        questions:         {rows:[],total:0},// 任务的问卷问题
    ...
    methods: {
       ...
       */
      isDisabaledDelBtn(){
        if(!this.common.checkAuthority('Lesson-Data-QuestionDetail-Disabled',true) === false){//有权限
       ...
       */
      isDisabaledPreBtn(){
        if(!this.common.checkAuthority('Lesson-Data-QuestionDetail-PreNext',true) === false){//有权限
      ...
      isDisabaledNexBtn(){
        if(!this.common.checkAuthority('Lesson-Data-QuestionDetail-PreNext',true) === false){//有权限
          if(this.exercisesIndex>this.exercises.questionCount-1){
            return true;
          }else{
            return false;
          }
      ...
      getQueryExercises(id){
        this.axios.get(`${this.common.basePath}/questionnaire/${id}`,'').then((response) => {
          this.exercises = response.data.data;
          response.data.data.id?this.getQueryQuestion(response.data.data.id):this.loading = false;;
        }).catch((error) => {
          this.$message.error(error);  
          this.loading = false;
        });
      ...
      getQueryQuestion(id){
        this.axios.get(`${this.common.basePath}/qitem`,{params: {questionnaire_id: id, sheet_id: null}}).then((response) => {  
