  ...
  <div class="content-wrap-box">
    ...
    <div class="router-view-wrap" v-loading="pageLoading">
      ...
      <div class="router-view-content">
        <el-row class="padding-row" v-if="common.checkAuthority('Out-list', true)">
          <!-- 入库搜索条件 -->
          <el-form ref="searchForm" :model="searchForm" label-width="120px" @submit.native.prevent>
            <el-row>
              ...
              <el-col :span="8">
                <el-form-item label=" ">
                  <el-button type="primary" @click="getInstockApplyList('init')" :disabled="!common.checkAuthority('Out-query', true)">搜索</el-button>
            ...
            <el-card shadow="">
              ...
              <el-table
                ...
                <el-table-column
                  ...
                  width="150">
                  <template slot-scope="scope">
                    <el-button type="" size="mini"  @click.stop='$router.push({name: "OutstockDetail", params: { type: 1, id: scope.row.id}});' v-if="common.checkAuthority('Out-detail', true)">查看</el-button>
                    <el-button type="success" size="mini" :disabled="scope.row.curStatus===2||scope.row.curStatus===3||scope.row.curStatus===5||scope.row.creator==me" @click.stop='$router.push({name: "OutstockDetailedForm", params: {type: 3, id: scope.row.id, page:1}});' v-if="common.checkAuthority('Out-approval', true)">审批</el-button>
                    <!-- <el-button type="primary" size="mini" :disabled="scope.row.curStatus===2||scope.row.curStatus===3||scope.row.curStatus===1" @click.stop='$router.push({name: "OutstockDetailedForm", params: {type: 2, id: scope.row.id, page:1}});' v-if="common.checkAuthority('Out-edit', true)">编辑</el-button>
                    <el-button type="danger" size="mini"  :disabled="scope.row.curStatus===2||scope.row.curStatus===3||scope.row.curStatus===5" v-if="common.checkAuthority('Out-recacll', true)"  @click.stop="deleteInstockApply(scope.row)">撤回</el-button> -->
                  </template>
                </el-table-column>
              </el-table>
              <!-- 分页 -->
      ...
      <div class="router-view-bottom_">
        <div class="overflow-hidden fill-width" v-if="common.checkAuthority('Out-list', true)">
          <el-button type="primary" size="" class="fr" @click='$router.push({name: "OutstockDetailedForm", params: {type: 1, id: 1, page:1 }});' :disabled="!common.checkAuthority('Out-create', true)">创建出库单</el-button>
        </div>
        <div v-else class="text-center text-info"> 你无权查看此内容 </div>
      </div>
...
<script>
  export default {
    ...
    mounted () {
      if(this.common.checkAuthority('Out-list', true)) {
        this.getUserList();
        this.getUserList_();
    ...
    methods: {
      ...
      getInstockApplyList(init) {
        ...
        let config = {
          method: "post",
          url: `${this.common.basePath}/materialApply/manager/list`,
