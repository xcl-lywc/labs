  ...
  <div class="content-wrap-box" ref="contentWrap">
    ...
    <el-card class="title-card">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        <el-breadcrumb-item>待提交报告列表</el-breadcrumb-item> 
      </el-breadcrumb>
    </el-card>
    <div class="router-view-wrap" ref="contentWrap">
      ...
      <div class="router-view-content">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" label-width="120px">
          ...
          <el-form-item>
            <el-button 
              :disabled="!common.checkAuthority('ReportTaskList_QueryList', true)"
              type="primary" @click="getQueryList(true)">
              {{common.checkAuthority('ReportTaskList_QueryList') ? common.checkAuthority('ReportTaskList_QueryList').name : ''}}
            </el-button>
          ...
          <el-col v-if="listData.rows.length" :span="8" v-for="(item, index) in listData.rows" :key="index">
            <el-card class="card-wrap" style="height: 327px;">
              <div class="card-content cursor-pointer" @click="goToDetail(item)">
                ...
                <p class="card-">报告类型：{{item.genre == 2 ? '个人报告' : '小组报告'}}</p>
...
<script>
  export default { 
    ...
    methods: {
      ...
      goToDetail(item) {
        this.$router.push({name: "ReportTaskDetail", params: {id: item.id, experiment_id: item.experiment_id}})
      },
      resetSearchForm(){ 
