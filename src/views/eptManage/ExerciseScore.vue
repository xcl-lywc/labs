  ...
  <div class="content-wrap-box" ref="contentWrap">
    ...
    <el-card class="title-card">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        <el-breadcrumb-item>习题评分</el-breadcrumb-item> 
      </el-breadcrumb>
    </el-card>
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" label-width="120px">
          ...
          <el-form-item>
            <el-button 
              ...
              {{common.checkAuthority('ExerciseScore_QueryList') ? common.checkAuthority('ExerciseScore_QueryList').name : ''}}
            </el-button>
        ...
        <el-table  
          ...
          <el-table-column 
            ...
            <template slot-scope="scope">
              <span @click="goToDetail(scope.row)" class="cursor-pointer">{{scope.row.name}}</span>
            </template>
          </el-table-column>
          <el-table-column
            width="150px" 
          ...
          <el-table-column   
            ...
            <template slot-scope="scope"> 
              <div>
                <!-- 状态为未完成 -->
                <el-button 
                  :disabled="!common.checkAuthority('ExerciseScore_Score', true)"
                  type="primary" size="mini"  
                  @click="showPaper(scope.row)">
                  {{common.checkAuthority('ExerciseScore_Score') ? common.checkAuthority('ExerciseScore_Score').name : ''}}
                </el-button>  
              </div>
            </template>
          </el-table-column>
...
<script> 
  export default { 
    ...
    methods: {
       ...
       */
      showPaper(item){ 
        this.$router.push({name: 'EPTMarkingScore', params:{ solution_id: item.solution_id }});
        this.activeAnswerItem = item; 
      },
      goToDetail(item) {
        this.$router.push({name: "ExerciseScoreDetail", params: {task_id: item.id, experiment_id: item.experiment_id}})
      },
    }
  }
</script>
